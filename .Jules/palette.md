...
## 2026-08-29 - [Form Interactions and Loading States]
**Learning:** Users can lose input data or trigger unexpected behaviors if interactive text inputs are not explicitly disabled during async operations, even if styled to look disabled. Additionally, empty states on submit buttons without explanations cause confusion.
**Action:** Always disable text inputs during loading states and provide native tooltip explanations for empty-state validations on submit buttons.
