...
## 2024-11-20 - Prevent Empty Prompt Submissions
**Learning:** Users can accidentally submit an empty prompt or a prompt that contains only whitespace, triggering unnecessary loading states and API calls. A disabled button alone isn't always enough context for the user to understand why it's disabled.
**Action:** When disabling a submit button due to missing input, add a `title` attribute explaining the requirement (e.g., "Enter a prompt to generate a component") to improve accessibility and UX. Also, disable the input field during async loading states to clearly indicate that new input is blocked until the current action completes.
