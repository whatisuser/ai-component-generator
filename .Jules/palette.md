...
## 2024-06-07 - Add disabled state to prompt textarea during generation
**Learning:** Textareas and inputs that are tied to async actions should be disabled during the action to prevent user confusion. If the `className` includes disabled styles (e.g. `disabled:cursor-not-allowed`), ensure the `disabled` attribute is actually conditionally applied. Adding an accessible tooltip (`title`) when disabled is a good practice to inform the user why they cannot interact with the input.
**Action:** Always check if form inputs have corresponding disabled states linked to the submission's `isLoading` state, and ensure both visual styling and accessible HTML attributes are present.
