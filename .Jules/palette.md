...
## 2024-05-02 - [Input Field Loading States]
**Learning:** Found that while the Tailwind classes for disabled states (`disabled:cursor-not-allowed disabled:bg-slate-50`) were predefined on the prompt textarea, the `disabled` property wasn't bound to the form's `isLoading` state. This allowed users to mutate input while an async generation request was already in flight, creating an inconsistent UI state.
**Action:** Always ensure both interactive elements (buttons and inputs) in a form are disabled when transitioning to a loading state to prevent concurrent mutations.
