...
## 2024-05-19 - Empty State Button Tooltips
**Learning:** Users often get confused when primary action buttons are disabled without context. Adding a `title` attribute to a disabled button explaining *why* it's disabled (e.g., "Enter a prompt to generate a component") provides immediate, accessible feedback.
**Action:** Always pair conditionally disabled buttons (like submit buttons that require input) with an explanatory tooltip or ARIA description.
