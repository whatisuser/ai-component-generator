...
## 2026-08-01 - Disable form elements appropriately and explain why
**Learning:** When form fields or submit buttons are disabled, it is critical for accessibility and UX to explain *why* they are disabled. Furthermore, textareas should also be disabled during loading states to prevent input while an operation is in progress, even if the submit button is already disabled.
**Action:** Add tooltips to disabled submit buttons explaining the required action (e.g. 'Enter a prompt'). Add the `disabled` prop to textareas during loading states.
