...
## 2024-05-19 - Hidden Text for Visual Indicators
**Learning:** When adding visual shortcuts inside buttons (like `⌘ ↵`), they can cause screen readers to announce confusing "Command Return" text embedded within the button's primary action. Since the action is already explained in nearby descriptive text ("Press ⌘/Ctrl + Enter to submit"), the inline visual shortcut indicator should be hidden from screen readers to prevent redundant, noisy announcements.
**Action:** Always add `aria-hidden="true"` to purely visual keyboard shortcut indicators that duplicate instructions already available to screen readers elsewhere in the context.
