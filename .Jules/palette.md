...
## 2026-08-09 - Disabling interactive elements correctly
**Learning:** When applying Tailwind `disabled:` classes to form elements, if the HTML/React `disabled` prop is not explicitly mapped, the elements remain interactive despite looking disabled, violating accessibility standards. Furthermore, when disabling interactive elements like submit buttons due to missing input, providing an explanatory native tooltip (e.g., using the `title` attribute) significantly improves accessibility and user understanding.
**Action:** Always verify that the HTML `disabled={...}` prop is explicitly mapped. When disabling submit buttons, add a `title` attribute explaining the required action.
