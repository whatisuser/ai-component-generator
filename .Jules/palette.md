...
## 2026-08-17 - Adding explanatory tooltips to disabled elements
**Learning:** When interactive elements (like a submit button) are disabled because of an empty input state, users might not intuitively know *why* they are disabled. Adding a `title` attribute to provide a native tooltip helps clarify the requirements.
**Action:** Always provide an explanatory native tooltip (e.g., using the `title` attribute) on disabled interactive elements when the reason for being disabled isn't immediately obvious from the surrounding UI state.
