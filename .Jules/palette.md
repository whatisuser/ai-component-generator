...
## 2024-06-15 - Lock the primary input alongside the submit button during async operations
**Learning:** When users trigger a long-running generation process (like AI component generation), they often assume that modifying the form input will either change the current request or invalidate it. If the primary text area remains active while the submit button shows "Generating...", users can become confused about the system state.
**Action:** When a form relies on a primary text input to submit async operations, always apply the disabled state to both the submit button and the main text input simultaneously to communicate that the current generation context is locked.
