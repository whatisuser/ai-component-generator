...
## 2026-06-25 - Using title tooltips on disabled interactive elements
**Learning:** Relying solely on `disabled` state can leave users confused as to *why* an action is disabled. Specifically mapping the disabled property explicitly and adding a native tooltip via the `title` attribute improves understanding.
**Action:** Whenever adding a disabled state to a button or interactive element, also include a `title` property explaining the condition (e.g. `title={!isValid ? "Explanation here" : undefined}`).
