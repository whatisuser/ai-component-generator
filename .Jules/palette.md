...
## 2025-01-09 - Mapping Tailwind disabled classes to HTML attributes
**Learning:** When using Tailwind CSS `disabled:` variant classes to style non-interactive states on form inputs (like `<textarea>`), the visual styling will *only* apply if the actual HTML `disabled` attribute is also bound correctly in React (e.g., `disabled={isLoading}`). Missing this mapping leads to an accessible/functional state mismatch where elements appear interactive but might not be, or vice versa.
**Action:** Always verify that Tailwind `disabled:` classes are explicitly mapped to the corresponding React `disabled={...}` prop on the HTML element to ensure it is both visually styled as disabled and functionally non-interactive for screen readers.
