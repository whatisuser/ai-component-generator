...
## 2024-05-01 - Functional Disabled States
**Learning:** Visual disabled styling (like Tailwind's `disabled:` pseudo-classes) does not prevent user interaction unless accompanied by the semantic HTML `disabled` attribute. The absence of the functional attribute allows users to continue interacting (e.g., typing in a textarea during loading).
**Action:** Always pair visual disabled styling with the corresponding functional HTML attribute (`disabled={...}`) to ensure forms and buttons are both visually and functionally disabled.
