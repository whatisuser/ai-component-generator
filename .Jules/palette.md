...
## 2026-07-20 - Explaining Disabled States
**Learning:** Disabling form inputs and buttons without context can leave users confused about how to proceed. While visual cues (like grayed-out styles) indicate the state, they don't explain the *reason*.
**Action:** When disabling interactive elements (like a submit button waiting for input or a form during a loading state), always provide an explanatory native tooltip using the `title` attribute to improve accessibility and user understanding of the disabled state.
