...
## 2026-09-04 - Prevent edits during async generation and explain disabled states
**Learning:** Users can accidentally lose edits if inputs like textareas remain active during async operations. Additionally, disabling buttons due to application state (like an empty input) without explanation can confuse users.
**Action:** Always disable text inputs (e.g. `disabled={isLoading}`) during form submissions. When disabling interactive elements, always provide a native tooltip via the `title` attribute to explain why the element is unavailable.
