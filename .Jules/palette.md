...
## 2024-08-25 - Disabling inputs during loading and explaining disabled states
**Learning:** When building forms, especially textareas that might take time to process, it's crucial to disable the input during the loading state (`disabled={isLoading}`) to prevent user edits from being lost. Furthermore, when disabling interactive elements like submit buttons because of missing input (empty prompt), providing an explanatory native tooltip (e.g., `title="Enter a prompt to generate"`) significantly improves accessibility and helps users understand why the action is blocked.
**Action:** Always ensure text inputs are disabled during loading states to preserve data integrity, and always add explanatory tooltips (`title` attribute) to disabled buttons when the reason might not be immediately obvious (e.g., missing input).
