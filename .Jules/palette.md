...
## 2024-05-20 - Prevent lost input and clarify disabled states
**Learning:** Users can lose input if they modify a textarea during an async operation, and a disabled submit button without an explanation leaves users confused about what action is required.
**Action:** Always ensure text inputs are disabled during loading states (e.g., disabled={isLoading}), and provide explanatory tooltips (title) on disabled interactive elements to improve accessibility and user understanding.
