...
## 2024-06-16 - Explicitly disable inputs during async submissions
**Learning:** During asynchronous form submissions, it's not enough to just disable the submit button or show a spinner. Since the user's focus is frequently still within the main input (like a prompt textarea), leaving it enabled creates a confusing state where they might continue typing or attempt a double-submission via a keyboard shortcut.
**Action:** When adding `isLoading` states to forms, ensure *all* major input fields—especially the primary textarea or input—are also explicitly disabled to provide immediate, unambiguous visual feedback and prevent simultaneous inputs.
