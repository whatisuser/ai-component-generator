...
## 2026-07-23 - Disabling Form Elements Appropriately
**Learning:** When disabling submit buttons due to an empty input or while a background task (like generation) is loading, providing a native tooltip via the `title` attribute explains *why* the action is disabled. Additionally, disabling the primary input fields (like textareas) during loading prevents users from making edits that won't be captured or might conflict with the ongoing task.
**Action:** Ensure inputs are explicitly disabled during async operations and use `title` on disabled buttons to explain the required state.
