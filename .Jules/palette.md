...
## 2024-05-25 - Empty State Validation for Submit Buttons
**Learning:** Users can easily miss validation requirements if the primary action button remains enabled but fails silently or throws an error after clicking. Disabling the submit button when required inputs are empty prevents invalid actions.
**Action:** When a form requires input, always disable the submit button if the input is empty or invalid, and provide a clear `title` attribute to explain why it is disabled to ensure accessibility.
