...
## 2026-07-13 - Ensuring disabled styles map to native props
**Learning:** When using utility classes like `disabled:bg-slate-400` or `disabled:opacity-50`, the styles will not apply unless the underlying HTML element actually has the `disabled` prop set. We often rely on application state (e.g. `isLoading`) to prevent interaction but forget to explicitly set the native disabled attribute.
**Action:** Always ensure that visual disabled states (using `disabled:` prefixes) are paired with the semantic HTML `disabled` prop based on the same conditions. Also, when disabling an element because of an empty input, provide a `title` tooltip so users understand why the action is blocked.
