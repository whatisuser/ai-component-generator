...
## 2026-07-25 - Improve disabled states on form controls
**Learning:** Users can be confused when actionable buttons are disabled without clear reason, or when fields remain editable while processing.
**Action:** Always provide explicit disabled states on interactive elements during loading/processing, and use the title attribute to provide context on why an element is disabled when the reason might not be immediately obvious.
