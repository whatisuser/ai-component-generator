...
## 2024-06-12 - Disabled State Feedback

**Learning:** When disabling submit buttons based on empty inputs, it's critical to provide a `title` attribute. Without it, users may wonder why they cannot interact with the form, especially if the input requirement isn't immediately obvious. Disabling form inputs (like `textarea`) during a loading state also prevents accidental keystrokes that might be lost or conflict with the incoming response.

**Action:** Always add a descriptive `title` attribute to buttons that are disabled conditionally (e.g., waiting for input). Always disable primary inputs (`textarea`, `input`) alongside the submit button during async loading states to lock the context.
