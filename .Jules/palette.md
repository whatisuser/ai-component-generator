...
## 2024-07-16 - Improving Form Validation & Async State Feedback

**Learning:** When forms are submitted via keyboard shortcuts or clicks, disabling the input fields (like textareas) during the async loading state prevents users from accidentally modifying data mid-flight. Furthermore, when disabling submit buttons due to invalid state (like an empty input), users often struggle to understand *why* the action is disabled.

**Action:** Always disable inputs while an async submission is processing. For submit buttons disabled due to invalid form state, include a native `title` tooltip attribute explaining the requirement (e.g., "Enter a prompt to generate"), improving accessibility and preventing user frustration.
