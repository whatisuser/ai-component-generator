...
## 2024-05-24 - [Accidental Garbage Files from Shell Output]
**Learning:** When looking at logs from `pnpm`, it often prefixes output lines with `>` (e.g. `> next dev` or `> ai-component-generator@0.1.0 dev`). When these logs are outputted, running them back into the terminal accidentally acts as a redirect creating empty garbage files (e.g., creating files named `next` and `local-dev-demo@0.1.0`).
**Action:** Be extremely careful not to accidentally paste shell command outputs containing `>` into new bash sessions without verifying the contents.
