...
## 2024-10-25 - Explanatory tooltips for disabled form submissions
**Learning:** When disabling interactive elements like submit buttons due to missing input or application state, provide an explanatory native tooltip (e.g., using the `title` attribute) to improve accessibility and user understanding of the disabled state.
**Action:** Always add an explanatory `title` attribute to submit buttons when they are disabled due to empty required fields or missing state.
