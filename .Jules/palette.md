...
## 2026-09-08 - Disable submit buttons on empty required inputs
**Learning:** When a form submission requires an input to not be empty, the submit button should be visually disabled with an explanatory tooltip, rather than triggering a post-click error message. This prevents unnecessary interactions and immediately communicates the requirement.
**Action:** Use the `disabled` property and a native `title` tooltip on submit buttons when the requisite fields are empty, ensuring the tooltip explains why the button is unavailable.
