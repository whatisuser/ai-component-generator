...
## 2026-07-29 - Adding explanatory tooltips to disabled interactive elements
**Learning:** When disabling interactive elements like submit buttons due to missing input, users can become frustrated if they don't understand why the action is blocked. Additionally, applying Tailwind `disabled:` classes without mapping the underlying HTML `disabled` prop causes accessibility issues as the element remains interactive.
**Action:** Always ensure the HTML `disabled` prop is explicitly mapped alongside Tailwind styling. For disabled buttons, provide an explanatory native tooltip (e.g., using the `title` attribute) to clearly communicate the missing requirements.
