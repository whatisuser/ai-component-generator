...
## 2024-05-24 - [Fix Missing Disabled Attributes on Loading Forms]
**Learning:** The prompt textarea had Tailwind `disabled:` classes configured for cursor and styling changes during loading, but was missing the actual HTML `disabled` attribute, allowing users to keep typing while generation was in progress. The submit button also lacked an empty state validation.
**Action:** Always ensure that when Tailwind `disabled:` utility classes are applied, the corresponding HTML `disabled={...}` prop is explicitly mapped. Also add native `title` tooltips to disabled buttons for better accessibility when an empty state prevents submission.
