...
## 2026-07-19 - Adding explicit disabled props and tooltips to Tailwind elements

**Learning:** Interactive elements styled with Tailwind's `disabled:` pseudo-class modifier (like `disabled:cursor-not-allowed` or `disabled:bg-slate-50`) require the explicit HTML `disabled` property to actually function accessibly and trigger those styles. Furthermore, providing an explanatory tooltip (via the `title` attribute) significantly enhances the UX by explaining *why* an input or button is disabled, rather than just leaving the user guessing.

**Action:** Whenever applying Tailwind `disabled:` classes to form elements or buttons, ensure the React/HTML `disabled={...}` prop is correctly mapped to state. Additionally, always include a `title` attribute to explain the disabled state.
