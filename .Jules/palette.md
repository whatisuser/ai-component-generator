...
## 2026-08-12 - Explicit Disabled Attributes and Tooltips
**Learning:** Tailwind `disabled:` classes are only visual; without the HTML `disabled` prop on inputs like `<textarea>`, the element remains interactive and screen readers are unaware of its state. Additionally, explaining *why* a button is disabled using a `title` tooltip reduces user friction for empty states.
**Action:** Always verify the HTML `disabled` prop is present alongside `disabled:` styling, and consistently add `title` tooltips to disabled action buttons.
