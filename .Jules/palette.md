...
## 2026-07-30 - Disabling interactive elements during async generation and empty states
**Learning:** Users can become confused or accidentally trigger errors when they interact with form inputs while a background process is running, or when they try to submit an empty form. Disabling inputs during loading and disabling submit buttons when required input is missing, paired with descriptive tooltips, prevents errors and communicates state clearly.
**Action:** Always map disabled state accurately to form elements during loading states and when required input is missing. Provide an explanatory native tooltip (e.g., using the `title` attribute) so users understand why an interaction is blocked.
