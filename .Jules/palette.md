...
## 2026-08-23 - Disabling inputs during loading and empty states
**Learning:** Users can lose input or get confused if textareas remain active during async operations, and buttons should be disabled when empty state prevents valid submission (with a tooltip explaining why).
**Action:** Always disable text inputs during loading states (`disabled={isLoading}`) and disable submit buttons when input is empty with an explanatory tooltip.
