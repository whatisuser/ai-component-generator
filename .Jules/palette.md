...
## 2024-05-14 - Playwright Next.js Hydration
**Learning:** Playwright may evaluate element attributes (like `disabled`) faster than React can hydrate the client components in a Next.js App Router application. This causes false negatives where Playwright sees an element as interactive before React attaches the `disabled` state derived from hooks.
**Action:** Always include a short delay (e.g., `time.sleep(2)`) or wait for specific application state indicators after `page.goto()` before asserting interactive UI states in local Playwright tests.
