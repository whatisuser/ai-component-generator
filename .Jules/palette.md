...
## 2025-01-20 - Adding explicit disabled states to inputs during async operations
**Learning:** While testing the prompt form, we observed that inputs were left interactive during component generation. In complex forms, ensuring that text inputs are disabled during loading states prevents user edits from being lost during async operations. We also added a title to the disabled submit button when no prompt is provided to explain why generation isn't available.
**Action:** When building forms, always bind the `disabled` state of inputs to the `isLoading` prop, and provide explanatory tooltips on empty-state disabled buttons.
