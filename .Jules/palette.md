...
## 2026-09-05 - Disabling forms and actionable tooltips
**Learning:** Disabling the text input while an async action runs prevents lost user edits and confusing state mismatches. Also, users need a clear tooltip explaining why an action button like Submit is disabled when the input is empty.
**Action:** When creating forms with async submissions, always ensure inputs are `disabled` during the loading state and that dynamic `title` attributes on submit buttons explain empty-state constraints.
