  ...
  return (
    ...
    >
      <div className="max-w-md text-center">
        <div
          className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-4 border-amber-200 border-t-amber-500"
          aria-hidden="true"
        />
        <h2 className="text-lg font-semibold">{title}</h2>
...
export function ErrorState({ title, description }: StateProps) {
  return (
    <section
      className="flex min-h-[20rem] flex-1 items-center justify-center rounded-[1.75rem] border border-rose-200 bg-rose-50 p-6 text-center text-rose-900"
      role="alert"
      aria-live="assertive"
    >
      <div className="max-w-md space-y-2">
